...
Fixes vs original draft:
  - change_requests.affected_features: ARRAY→Text (model stores JSON-encoded list)
  - change_requests.project_id FK: CASCADE→SET NULL (matches model)
  - All server_default string literals wrapped in sa.text() for dialect safety
  - Consistent Uuid() column type (matches SQLAlchemy Uuid dialect-agnostic type)
"""
...
def upgrade() -> None:
    ...
    # ── change_requests ───────────────────────────────────────────────────────
    # affected_features is a TEXT column storing a JSON-encoded list of strings.
    # The ORM model uses Text (not ARRAY) so it stays portable across PostgreSQL
    ...
    op.create_table(
        ...
        sa.Column('affected_features', sa.Text(), nullable=True),   # JSON-encoded list
        ...
        sa.ForeignKeyConstraint(['project_id'], ['projects.id'], ondelete='SET NULL'),  # SET NULL matches model
        sa.PrimaryKeyConstraint('id'),
    )
    ...
    op.create_table(
        ...
        sa.Column('drift_events', sa.Integer(), nullable=False, server_default=sa.text('0')),
        sa.Column('contradiction_events', sa.Integer(), nullable=False, server_default=sa.text('0')),
        sa.Column('stability_score', sa.Float(), nullable=False, server_default=sa.text('100.0')),
        sa.ForeignKeyConstraint(['client_id'], ['users.id'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['project_id'], ['projects.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
    )
    ...
    op.create_table(
        ...
        sa.Column('message_type', sa.String(50), nullable=False, server_default=sa.text("'normal'")),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('embedding_version', sa.String(30), nullable=True),
        sa.Column('status', sa.String(50), nullable=False, server_default=sa.text("'active'")),
    ...
    op.create_table(
        ...
        sa.Column('generated_by', sa.String(50), nullable=False, server_default=sa.text("'system'")),
